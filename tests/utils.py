...
def display_annotated(code, class_binds_near):
    ...
    for node, scope in mapping.items():
        ...
        lines[node.lineno - 1][node.col_offset] = (
            "{" + scope_description + "}" + lines[node.lineno - 1][node.col_offset]
        )
...
class DisplayAnnotatedTestCase(unittest.TestCase):
    def _check_nodes(self, mapping, *scopes):
        overall_scope = [
            item for scope in scopes for item in all_nodes_gen_for_scope(scope)
        ]
    ...
    def assertAnnotationWorks(
        self, annotated_code, code=None, *, class_binds_near=False
    ):
        ...
        self._check_nodes(
            ...
            scope_info._error_scope,
        )

        self.assertEqual(
            display_annotated(code, class_binds_near), trim(annotated_code)
        )
