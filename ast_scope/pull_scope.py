...
from .scope import GlobalScope, ErrorScope, FunctionScope, ClassScope, Scope
...
class PullScopes(GroupSimilarConstructsVisitor):
    def __init__(self, annotation_dict: dict[ast.AST, tuple[str, IntermediateScope, bool]]):
    ...
    def visit_function_def(self, func_node: ast.FunctionDef | ast.AsyncFunctionDef, is_async: bool):
        ...
        if func_node not in self.node_to_corresponding_scope:
            self.node_to_corresponding_scope[func_node] = FunctionScope(func_node, scope)
        scope.add_function(
            func_node, self.node_to_corresponding_scope[func_node], include_as_variable=True
        )
