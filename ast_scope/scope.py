...
class Variables:
    ...
    functions: set[ast.FunctionDef | ast.Lambda | ast.comprehension | ast.AsyncFunctionDef] = attr.ib(factory=set)
    ...
    def node_to_symbol(self):
        ...
        result.update({var: var.name for var in self.functions if isinstance(var, (ast.FunctionDef, ast.AsyncFunctionDef))})
...
class Scope(abc.ABC):
    ...
    def add_function(self, node: ast.FunctionDef | ast.Lambda | ast.comprehension | ast.AsyncFunctionDef, function_scope: Self, include_as_variable: bool):
...
class FunctionScope(ScopeWithChildren, ScopeWithParent):
    def __init__(self, function_node: ast.FunctionDef | ast.Lambda | ast.comprehension | ast.AsyncFunctionDef, parent: Scope):
